...
async function maybeCreateStream(localView) {
...
async function postMasterLogin(localView, remoteView, formValues, onStatsReport, onRemoteDataMessage){
    ...
    try {
        ...
        localView.oncanplay = maybeCreateStream(localView)
        if (localView.readyState >= 3) { // HAVE_FUTURE_DATA
            // Video is already ready to play, call maybeCreateStream in case oncanplay
            // fired before we registered the event handler.
            maybeCreateStream(localView);
        }
        localView.play();
    } catch (e) {
        console.error('[MASTER] Could not find video');
    }
